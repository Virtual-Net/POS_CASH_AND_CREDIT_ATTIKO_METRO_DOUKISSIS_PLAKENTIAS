...
namespace eSSP_example
{
    public class CNV11
    {
                ...
                switch (m_CurrentPollResponse[i])
                {
                    ...
                    case CCommands.SSP_POLL_READ_NOTE:
                        if (m_CurrentPollResponse[i + 1] > 0)
                        {
                            ...
                            log.AppendText("Note in escrow, amount: " + CHelpers.FormatToCurrency(noteVal) + " " + GetChannelCurrency(m_CurrentPollResponse[i + 1]) + "\r\n");
                    ...
                    case CCommands.SSP_POLL_CREDIT_NOTE:
                        noteVal = GetChannelValue(m_CurrentPollResponse[i + 1]);
                        log.AppendText("Credit " + CHelpers.FormatToCurrency(noteVal) + " " + GetChannelCurrency(m_CurrentPollResponse[i + 1]) + "\r\n");
                        ...
                        switch(noteVal)
                        {
                            case 500:
                                ...
                                break;
                            case 1000:
                                storedNoteValue = 1000;
                                break;
                            case 2000:
                                storedNoteValue = 2000;
                                break;
                            case 5000:
                                storedNoteValue = 5000;
                                break;
                        }
                        NotesAccepted++;
                        UpdateData();
                    ...
                    case CCommands.SSP_POLL_DISABLED:
                        if (!(log.Lines.Last() == "Unit disabled...\r\n"))
                        {
                    ...
                    case CCommands.SSP_POLL_NOTE_CLEARED_FROM_FRONT:
                        log.AppendText(GetChannelValue(m_CurrentPollResponse[i + 1]) + " " + GetChannelCurrency(m_CurrentPollResponse[i + 1]) + " note cleared from front at reset." + "\r\n");
                        i++;
                        break;
                    ...
                    case CCommands.SSP_POLL_NOTE_CLEARED_TO_CASHBOX:
                        log.AppendText(GetChannelValue(m_CurrentPollResponse[i + 1]) + " " + GetChannelCurrency(m_CurrentPollResponse[i + 1]) + " note cleared to stacker at reset." + "\r\n");
                        i++;
                        break;
        ...
        {
            string notesinstorage = "";
            StringBuilder sb = new StringBuilder("", 100);
            int i;
            sb.Append(m_StoredCurrency);
            sb.Append("\r\n");
            for (i = 0; i < m_NotePositionValues.Length; i++)
            {
            ...
            if (sb == null || sb.ToString().Equals(""))
                notesinstorage = "0";
            return notesinstorage;//sb.ToString();
        }
