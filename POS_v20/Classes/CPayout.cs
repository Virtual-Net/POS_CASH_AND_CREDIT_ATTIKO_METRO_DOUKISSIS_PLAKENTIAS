    ...
    public class CPayout
    {
        ...
        public string GetCashboxPayoutOpData(TextBox log = null)
        {
            ...
            if (CheckGenericResponses(log))
            {
                ...
                if (log != null) log.AppendText(sbDisplay.ToString());
        ...
        public Tuple<bool,int> DoPoll(TextBox log)
        {
            byte i;
            int m_PollResponse = 0;
            // If a note is to be held in escrow, send hold commands, as poll releases note.
            if (m_HoldCount > 0)
            {
                m_NoteHeld = true;
                m_HoldCount--;
            ...
            //m_cmd.ResponseData.CopyTo(m_PollResponse, 0);
            //MessageBox.Show(Encoding.ASCII.GetString(m_PollResponse, 0, m_PollResponse.Length));
                ...
                switch (response[i])
                {
                    ...
                    case CCommands.SSP_POLL_READ_NOTE:
                        if (m_cmd.ResponseData[i + 1] > 0)
                        {
                            ...
                            string PaidNote = CHelpers.FormatToCurrency(data.Value);
            ...
            return Tuple.Create(true,m_PollResponse);
        }
